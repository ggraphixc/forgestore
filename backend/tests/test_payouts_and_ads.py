...
- Flutterwave webhook routing for ad payments
"""
...
# ==============================================================================
# TEST CASE 5: Flutterwave Webhook Routing for Ad Payments
# ==============================================================================


class TestFlutterwaveAdWebhook:
    """POST /api/flutterwave/webhook — Ad campaign payment detection."""
    ...
    def _set_encryption_key(self, monkeypatch):
        """Set the Flutterwave encryption key so verif-hash comparison passes."""
        monkeypatch.setattr(
            "app.routers.flutterwave_webhook.get_settings",
            lambda: type("obj", (object,), {"flutterwave_encryption_key": self.ENCRYPTION_KEY})(),
        )

    def test_webhook_marks_campaign_paid(self, client, db, pending_ad_campaign):
        """charge.completed webhook with campaign reference transitions PENDING→PAID."""
        reference = pending_ad_campaign.payment_reference

        # Note: Flutterwave webhook validates order_id BEFORE checking for campaigns.
        # Unlike Paystack (which checks campaigns first), Flutterwave requires a non-empty
        # order_id or a matching Order.order_number to proceed to campaign detection.
        # Use a dummy order_id to pass the validation gate.
        payload = {
            "event": "charge.completed",
            "data": {
                "status": "successful",
                "tx_ref": reference,
                "meta": {"order_id": "ad-campaign-payment"},
            },
        }
        ...
        resp = client.post(
            self.ENDPOINT,
            json=payload,
            headers={"verif-hash": self.ENCRYPTION_KEY},
        )
        ...
        assert notif is not None
        assert "Ad Campaign" in notif.title
        assert "Flutterwave" in notif.message

    def test_webhook_ignores_non_ad_transaction(self, client, db, pending_ad_campaign):
        """charge.completed with non-campaign reference is processed as usual."""
        payload = {
            "event": "charge.completed",
            "data": {
                "status": "successful",
                "tx_ref": "NON-AD-REF-002",
                "meta": {"order_id": "order-456"},
            },
        }
        ...
        assert resp.status_code in (200, 404)  # Order won't exist

        # Our campaign remains PENDING
        db.refresh(pending_ad_campaign)
        assert pending_ad_campaign.status == "PENDING"

    def test_webhook_invalid_verif_hash_returns_401(self, client):
        """Invalid verif-hash header returns 401."""
        payload = {
            "event": "charge.completed",
            "data": {"status": "successful", "tx_ref": "ref-001"},
        }
    ...
    def test_webhook_ignores_unsuccessful_transaction(self, client, db, pending_ad_campaign):
        """charge.completed with failed status is ignored."""
        payload = {
            "event": "charge.completed",
            "data": {
                "status": "failed",
                "tx_ref": pending_ad_campaign.payment_reference,
            },
        }
    ...
    def test_webhook_ignores_wrong_event_type(self, client, db, pending_ad_campaign):
        """Non charge.completed/transfer.completed events are ignored."""
        payload = {
